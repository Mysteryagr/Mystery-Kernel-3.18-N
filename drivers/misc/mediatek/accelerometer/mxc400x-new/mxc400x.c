...
static struct i2c_board_info i2c_mxc400x __initdata = {I2C_BOARD_INFO(MXC400X_DEV_NAME, MXC400X_I2C_ADDR)};
...
{
	if (mt_set_gpio_mode(GPIO_GSE_SDA_PIN, GPIO_GSE_SDA_PIN_M_GPIO))
		GSE_LOG("MXC400X cust_i2c_sda_dir_set mode failed\n");
	if (mode == GPIO_DIR_IN) {
		if (mt_set_gpio_dir(GPIO_GSE_SDA_PIN, GPIO_DIR_IN))
			GSE_LOG("MXC400X cust_i2c_sda_dir_set in failed\n");
	} else{
		if (mt_set_gpio_dir(GPIO_GSE_SDA_PIN, GPIO_DIR_OUT))
			GSE_LOG("MXC400X cust_i2c_sda_dir_set out failed\n");
	}
}
...
{
	if (mt_set_gpio_mode(GPIO_GSE_SDA_PIN, GPIO_GSE_SDA_PIN_M_GPIO))
		GSE_LOG("MXC400X cust_i2c_sda_get mode failed\n");
	if (mt_set_gpio_dir(GPIO_GSE_SDA_PIN, GPIO_DIR_IN))
		GSE_LOG("MXC400X cust_i2c_sda_get dir failed\n");
...
char cust_i2c_write_byte(unsigned char addr, unsigned char regaddr, unsigned char udata)
{
	...
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_write_byte device addr error\n");
		return -1;
	}
	cust_i2c_send_byte(regaddr);
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_write_byte reg addr error\n");
		return -1;
	}
	cust_i2c_send_byte(udata);
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_write_byte reg data error\n");
		return -1;
	}
	cust_i2c_stop();
	return 0;
...
char cust_i2c_read_byte(unsigned char addr, unsigned char regaddr, unsigned char *udata)
{
	...
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_read_byte device addr error\n");
		return -1;
	}
	cust_i2c_send_byte(regaddr);
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_read_byte reg addr error\n");
		return -1;
	}
	cust_i2c_start();
	addr |= 0x01;
	...
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_read_byte devce addr error\n");
		return -1;
	}
...
char cust_i2c_write_bytes(unsigned char addr, unsigned char regaddr, unsigned char *udata, unsigned int count)
{
	...
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_write_bytes device addr error\n");
		return -1;
	}
	cust_i2c_send_byte(regaddr);
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_write_bytes reg addr error\n");
		return -1;
	}
	for (i = 0; i < count; i++) {
		...
		res = cust_i2c_get_ack();
		if (0 != res) {
			GSE_LOG("MXC400X cust_i2c_write_bytes reg data error\n", __func__, __LINE__);
			return -1;
		}
	}
	cust_i2c_stop();
...
char cust_i2c_read_bytes(unsigned char addr, unsigned char regaddr, unsigned char *udata, unsigned int count)
{
	...
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_read_bytes device addr error\n");
		return -1;
	}
	cust_i2c_send_byte(regaddr);
	res = cust_i2c_get_ack();
	if (0 != res) {
		GSE_LOG("MXC400X cust_i2c_read_bytes reg addr error\n");
		return -1;
	}
	cust_i2c_start();
	addr |= 0x01;
	...
	for (i = 0; i < count-1; i++) {
		...
		if (0 != res) {
			GSE_LOG("MXC400X cust_i2c_read_bytes reg data error\n");
			return -1;
		}
	}
	udata[i] = cust_i2c_get_byte();
...
int cust_i2c_master_read(struct i2c_client *client, u8 *buf, u8 count)
{
	...
	reg_addr = buf[0];
#if defined(SW_SIMULATION_DEBUG)
	GSE_LOG("MXC400X cust_i2c_master_read_byte reg_addr=0x %x\n", reg_addr);
#endif
...
int cust_hwmsen_write_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	reg_addr = addr;
#if defined(SW_SIMULATION_DEBUG)
	GSE_LOG("MXC400X cust_hwmsen_write_block reg_addr=0x%x\n", reg_addr);
#endif
...
int cust_hwmsen_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	mutex_unlock(&g_gsensor_mutex);
#if defined(SW_SIMULATION_DEBUG)
	GSE_LOG("MXC400X cust_hwmsen_read_block addr=0x%x, buf=0x%x\n", addr, buf[0]);
#endif
	memcpy(data, buf, len);
	return 0;
}
...
err = cust_hwmsen_read_block(client, addr, buf, MXC400X_DATA_LEN);
	else if (err)
#else
...
#if defined(SW_SIMULATION)
err = cust_hwmsen_read_block(client, addr, &temp, 0x01);
else if (err)
#else
...
#if defined(SW_SIMULATION)
err = cust_hwmsen_read_block(client, addr, buf, MXC400X_DATA_LEN+1);
	else if (err)
#else
			...
			GSE_LOG("Gsensor device enable function enable = %d, sensor_power = %d!\n", value, sensor_power);
				if (((value == 0) && (sensor_power == false)) || ((value == 1) && (sensor_power == true))) {
					...
					if (atomic_read(&priv->suspend) == 0) {
						...
						GSE_LOG("not in suspend MXC400X_SetPowerMode!, enable_status = %d\n", enable_status);
					} else
							GSE_LOG("in suspend can not enable or disable!enable_status = %d\n", enable_status);
				}
				mutex_unlock(&mxc400x_mutex);
			 }
			 break;
...
const struct file_operations mxc400x_fops = {
		 /*//.owner = THIS_MODULE,*/
		 .open = mxc400x_open,
...
#ifdef CONFIG_CUSTOM_KERNEL_SENSORHUB
	INIT_WORK(&obj->irq_work, mxc400x_irq_work);
...
static int mxc400x_i2c_remove(struct i2c_client *client)
{
	int err = 0;
	if (0 !=
	    (err = mxc400x_delete_attr(&(kxtj2_1009_init_info.platform_diver_addr->driver)))) {
		GSE_ERR("mxc400x_delete_attr fail: %d\n", err);
	}

	if (0 != (err = misc_deregister(&mxc400x_device))) {
